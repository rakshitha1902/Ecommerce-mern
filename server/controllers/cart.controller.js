...
const findUserCart = async (req, res) => {
  ...
  const clearUserCart =  async (req, res) => {
    try {
      ...
      res.status(200).json({ message: "User's cart has been cleared." });
    } catch (error) {
      // Handle error here and send an appropriate response
      res.status(500).json({ message: "Failed to clear user's cart.", error: error.message });
    }
  }

  module.exports={findUserCart,addItemToCart,clearUserCart};